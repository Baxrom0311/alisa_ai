...
def get_optimal_microphone():
    print("[TIZIM] Uskunalar tekshirilmoqda...")
    pa = pyaudio.PyAudio()
    best_idx = None
    best_rate = 48000

    # Tashqi mikrofonga xos nomlar
    external_keywords = [
        "usb",
        "webcam",
        "headset",
    ...
    ignore_keywords = ["stereo mix", "virtual", "output", "loopback"]
    ...
    # 1) Avval default'dan boshqa, nomi tashqi mikrofonga o'xshash qurilmani tanlaymiz
    for info in input_devices:
        idx = int(info["index"])
        name = info["name"].lower()
        if any(bad in name for bad in ignore_keywords):
            continue
        if any(kw in name for kw in external_keywords) and idx != default_idx:
            best_idx = idx
            best_rate = int(info["defaultSampleRate"])
            print(f" Tashqi mikrofon tanlandi: {info['name']} (ID:{best_idx})")
            break

    # 2) Agar topilmasa, default input
    if best_idx is None and default_info is not None:
        best_idx = default_idx
        best_rate = int(default_info["defaultSampleRate"])
        print(f" Standart mikrofon tanlandi: {default_info['name']} (ID:{best_idx})")

    # 3) Oxirgi fallback: ro'yxatdagi birinchi input
    if best_idx is None and input_devices:
        fallback = input_devices[0]
        best_idx = int(fallback["index"])
        best_rate = int(fallback["defaultSampleRate"])
        print(f" Fallback mikrofon tanlandi: {fallback['name']} (ID:{best_idx})")

    pa.terminate()
    if best_idx is None:
        print(" Mikrofon topilmadi. Tizim avtomatik qurilmasi ishlatiladi.")
...
async def speak(text):
    ...
    try:
        ...
        print(f"[XATO] {FALLBACK_TTS_ERROR}")
...
async def main():  # main funksiyasini async qildim
    ...
    try:
        mic = sr.Microphone(device_index=mic_idx, sample_rate=mic_rate)
    except Exception as e:
